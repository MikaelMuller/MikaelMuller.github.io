...
function enviarRegistro(eventId) {
  ...
  if(document.getElementById("nome").value == "" || document.getElementById("idade").value == "" || document.getElementById("enderecoEmail").value == "" || document.getElementById("telefone").value == ""){
    ...
    document.getElementById("avisoTexto").style = "color: white";
...
DB.ref("eventos").once("value").then(function(snapshot) {
  ...
  snapshot.forEach(function(childSnapshot) {
    ...
    if(data.formulario){
      if (Date.parse(data.dia) > Date.now() + 54000000) {
        ...
        if(vagasLivres > 0){
          meusEventos = meusEventos + '{"start": "' + data.dia + '", "title": "'+ data.nome + '", "id": "' + data.id + '", "backgroundColor": "'+data.cor+'", "extendedProps": {"texto": '+ JSON.stringify(data.texto) + ', "vagas": "'+vagasLivres+'", "passou": false, "formulario": true}},';
        } else{
          meusEventos = meusEventos + '{"start": "' + data.dia + '", "title": "'+ data.nome + '", "id": "' + data.id + '", "backgroundColor": "gray", "extendedProps": {"texto": '+ JSON.stringify(data.texto) + ', "vagas": "'+vagasLivres+'", "passou": false, "formulario": true}},'
        }
      } else{
        meusEventos = meusEventos + '{"start": "' + data.dia + '", "title": "'+ data.nome + '", "id": "' + data.id + '", "backgroundColor": "gray","extendedProps": {"texto": '+ JSON.stringify(data.texto) + ', "vagas": "0", "passou": true, "formulario": true}},';
      }
    } else{
      meusEventos = meusEventos + '{"start": "' + data.dia + '", "title": "'+ data.nome + '", "id": "' + data.id + '", "backgroundColor": "'+data.cor+'","extendedProps": {"texto": '+ JSON.stringify(data.texto) + ', "vagas": "0", "passou": false, "formulario": false}},';
    }
  });
